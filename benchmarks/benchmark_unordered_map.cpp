        ...
        switch (op) {
            ...
            case 1: // Find
                if (!keys.empty()) {
                    ...
                    benchmark::DoNotOptimize(unordered_map_find(map, &keys[idx]));
                }
                break;
            case 2: // Erase
                if (!keys.empty()) {
                    size_t idx = gen() % keys.size();
                    benchmark::DoNotOptimize(unordered_map_erase(map, &keys[idx]));
...
static void BM_UnorderedMapCollisions(benchmark::State& state) {
    ...
    for (int i = 0; i < state.range(0); ++i) {
        ...
        colliding_keys.push_back(base + "bb"); // Will likely collide due to similar pattern
    }
    ...
    for (auto _ : state) {
        ...
        for (int i = 0; i < colliding_keys.size(); ++i) {
            benchmark::DoNotOptimize(unordered_map_insert(map, &colliding_keys[i], &i));
        }
    }
