        ...
        switch (op) {
            ...
            case 1: // Find
                if (!elements.empty()) {
                    ...
                    benchmark::DoNotOptimize(unordered_set_find(set, &elements[idx]));
                }
                break;
            case 2: // Erase
                if (!elements.empty()) {
                    size_t idx = gen() % elements.size();
                    benchmark::DoNotOptimize(unordered_set_erase(set, &elements[idx]));
...
static void BM_UnorderedSetCollisions(benchmark::State& state) {
    ...
    for (int i = 0; i < state.range(0); ++i) {
        ...
        colliding_strings.push_back(base + "bb"); // Will likely collide due to similar pattern
    }
