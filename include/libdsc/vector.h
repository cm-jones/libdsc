...
///
/// @see vector_create()
void vector_destroy(DSCVector *vec);

/// @brief Returns the number of elements in the vector
///
/// @param vec Pointer to the vector (can be NULL)
/// @return Number of elements currently stored in the vector, or 0 if vec is
/// NULL
///
...
/// ```
size_t vector_size(DSCVector const *vec);
...
bool vector_empty(DSCVector const *vec);
...
size_t vector_capacity(DSCVector const *vec);
...
DSCError vector_reserve(DSCVector *vec, size_t n);
...
/// }
/// ```
DSCError vector_push_back(DSCVector *vec, void const *element);
...
DSCError vector_pop_back(DSCVector *vec);

/// @brief Returns a pointer to the element at the specified index
///
/// @param vec Pointer to the vector (must not be NULL)
/// @param index Index of the element to access
/// @return Pointer to the element, or NULL if index is out of bounds or vec is
/// NULL
///
/// @note This operation is O(1)
...
void *vector_back(DSCVector *vec);
...
void vector_clear(DSCVector *vec);
